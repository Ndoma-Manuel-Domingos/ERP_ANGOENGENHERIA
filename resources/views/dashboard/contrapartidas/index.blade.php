                            ...
                            <div class="card-header">
                                <h3 class="card-title">
                                    @if (Auth::user()->can('criar conta'))
                                        <a href="{{ route('contrapartidas.create') }}" class="btn btn-sm btn-primary">Nova Contropartida</a>
                                    @endif
                                </h3>
                                                        ...
                                                        <div class="btn-group">
                                                            ...
                                                            <div class="dropdown-menu" role="menu">
                                                                @if (Auth::user()->can('listar conta'))
                                                                    <a class="dropdown-item" href="{{ route('contrapartidas.show', $item->id) }}"><i class="fas fa-eye text-info"></i> Detalhes</a>
                                                                @endif
                                                                @if (Auth::user()->can('editar conta'))
                                                                    <a class="dropdown-item" href="{{ route('contrapartidas.edit', $item->id) }}"><i class="fas fa-edit text-success"></i> Editar</a>
                                                                @endif
                                                                <div class="dropdown-divider"></div>
                                                                @if (Auth::user()->can('eliminar conta'))
                                                                    <form
                                                                        action="{{ route('contrapartidas.destroy', $item->id) }}"
                                                                        method="post">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit"
                                                                            class="btn btn-sm btn-danger dropdown-item"
                                                                            onclick="return confirm('Tens Certeza que Desejas excluir Esta Contrapartidas?')">
