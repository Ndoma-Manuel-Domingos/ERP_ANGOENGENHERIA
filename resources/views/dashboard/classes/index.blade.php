                            ...
                            <div class="card-header">
                                <h3 class="card-title">
                                    @if (Auth::user()->can('criar classe'))
                                        <a href="{{ route('classes.create') }}" class="btn btn-sm btn-primary">Nova Classe</a>
                                    @endif
                                </h3>
                                                            ...
                                                            <div class="dropdown-menu" role="menu">
                                                                @if (Auth::user()->can('listar classe'))
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('classes.show', $item->id) }}"><i
                                                                            class="fas fa-eye text-info"></i> Detalhes</a>
                                                                @endif
                                                                @if (Auth::user()->can('editar classe'))
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('classes.edit', $item->id) }}"><i
                                                                            class="fas fa-edit text-success"></i> Editar</a>
                                                                @endif
                                                                <div class="dropdown-divider"></div>
                                                                @if (Auth::user()->can('eliminar classe'))
                                                                    <form
                                                                        action="{{ route('classes.destroy', $item->id) }}"
                                                                        method="post">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit"
                                                                            class="btn btn-sm btn-danger dropdown-item"
                                                                            onclick="return confirm('Tens Certeza que Desejas excluir esta classe?')">
