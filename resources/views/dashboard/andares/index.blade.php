                            ...
                            <div class="card-header">
                                <h3 class="card-title">
                                    @if (Auth::user()->can('criar andar'))
                                        <a href="{{ route('andares.create') }}" class="btn btn-sm btn-primary">Novo Andar</a>
                                    @endif
                                </h3>
                                                            ...
                                                            <div class="dropdown-menu" role="menu">
                                                                @if (Auth::user()->can('listar andar'))
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('andares.show', $item->id) }}"><i
                                                                            class="fas fa-eye text-info"></i> Detalhes</a>
                                                                @endif
                                                                @if (Auth::user()->can('editar andar'))
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('andares.edit', $item->id) }}"><i
                                                                            class="fas fa-edit text-success"></i> Editar</a>
                                                                @endif
                                                                <div class="dropdown-divider"></div>
                                                                @if (Auth::user()->can('eliminar andar'))
                                                                    <form
                                                                        action="{{ route('andares.destroy', $item->id) }}"
                                                                        method="post">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit"
