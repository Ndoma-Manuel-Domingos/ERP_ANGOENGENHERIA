                            ...
                            <div class="card-header">
                                <h3 class="card-title">
                                    @if (Auth::user()->can('criar quarto'))
                                        <a href="{{ route('quartos.create') }}" class="btn btn-sm btn-primary">Novo Quarto</a>
                                    @endif
                                </h3>
                                                    ...
                                                    <td class="text-right">
                                                        <div class="btn-group">
                                                            ...
                                                            <div class="dropdown-menu" role="menu">
                                                                @if (Auth::user()->can('listar quarto'))
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('quartos.show', $item->id) }}"><i
                                                                            class="fas fa-eye text-info"></i> Detalhes</a>
                                                                @endif
                                                                @if (Auth::user()->can('editar quarto'))
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('quartos.edit', $item->id) }}"><i
                                                                            class="fas fa-edit text-success"></i> Editar</a>
                                                                @endif
                                                                <div class="dropdown-divider"></div>
                                                                @if (Auth::user()->can('eliminar quarto'))
                                                                    <form
                                                                        action="{{ route('quartos.destroy', $item->id) }}"
                                                                        method="post">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit"
