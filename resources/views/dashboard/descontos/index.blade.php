                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">Descontos</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-12 col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">
                                    @if (Auth::user()->can('criar desconto'))
                                        <a href="{{ route('descontos.create') }}" class="btn btn-sm btn-primary">Novo
                                            Desconto</a>
                                    @endif
                                </h3>
                                                            ...
                                                            <div class="dropdown-menu" role="menu">
                                                                @if (Auth::user()->can('listar desconto'))
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('descontos.show', $item->id) }}"><i
                                                                            class="fas fa-eye text-info"></i> Detalhes</a>
                                                                @endif
                                                                @if (Auth::user()->can('editar desconto'))
                                                                    <a class="dropdown-item"
                                                                        href="{{ route('descontos.edit', $item->id) }}"><i
                                                                            class="fas fa-edit text-success"></i> Editar</a>
                                                                @endif
                                                                <div class="dropdown-divider"></div>
                                                                @if (Auth::user()->can('eliminar desconto'))
                                                                    <form
                                                                        action="{{ route('descontos.destroy', $item->id) }}"
                                                                        method="post">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit"
                                                                            class="btn btn-sm btn-danger dropdown-item"
                                                                            onclick="return confirm('Tens Certeza que Desejas excluir esta subsídio?')">
